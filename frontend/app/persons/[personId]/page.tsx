  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-xl border border-surface-border bg-surface-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">Face Recognition</h2>
            <p className="text-xs text-gray-500 mt-1">
              Upload a clear, front-facing photo. The face embedding is extracted and saved — the photo itself is not stored.
              Once trained, this person will be automatically identified in future batch uploads.
            </p>
          </div>
          {hasEmbedding && (
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-navy border border-surface-border text-sm text-white hover:border-brand-gold/60 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            <Upload className="w-4 h-4" />
            {uploading ? "Processing..." : hasEmbedding ? "Replace Photo" : "Upload Photo"}
          </button>
